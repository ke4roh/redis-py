...
class ObjectRedisTTL(ObjectRedis):
    ...
    def __init__(self, ttl, missing=_missing, redis=redis.StrictRedis(),
                 namespace=None, serializer=pickle,
        ...
        :param namespace: Prepended to keys, None to prepend nothing.  If
            namespace is none, then all contents of the database are considered
            members of the collection and processed through the serializers.
            The namespace will be encoded as bytes (str(ns).encode('utf-8'))
            if it's not already a byte array
        ...
        """
        super(ObjectRedisTTL, self).__init__(redis=redis, namespace=namespace,
